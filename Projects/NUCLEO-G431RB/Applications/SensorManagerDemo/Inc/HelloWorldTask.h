...
struct _HelloWorldTask {
  ...
  /**
   * In this demo the User Button and the sensor share the same EXTI.
   * After a PM state transaction to STATE1 we need to enable again teh IRQ.
   */
  boolean_t ub_irq_enabled;
};
...
/**
 * Select a sensor to be used with the demo and return its ID.
 * This function can be modified to select a differ sensor.
 *
 * @param _this [IN] specifies a pointer to a task object.
 * @return the ID of the selected sensor or SI_NULL_SENSOR_ID if a problem occurred.
 */
