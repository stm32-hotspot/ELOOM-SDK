...
sys_error_code_t IIS3DWBTask_vtblOnCreateTask(AManagedTask *_this, TaskFunction_t *pvTaskCode, const char **pcName, unsigned short *pnStackDepth,
...
//    if(iis3dwb_wtm_level < IIS3DWB_MIN_WTM_LEVEL)
//    {
//      iis3dwb_wtm_level = IIS3DWB_MIN_WTM_LEVEL;
//    }

    SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("**** IIS3DWB: REAL FIFO WM: %d\r\n", iis3dwb_wtm_level));
...
static sys_error_code_t IIS3DWBTaskCreateTimer(IIS3DWBTask *_this)
{
  ...
  _this->read_timer = xTimerCreate("IIS3DWB_T", pdMS_TO_TICKS(IIS3DWB_TASK_CFG_TIMER_PERIOD_MS), pdFALSE, _this, IIS3DWBTaskTimerCallbackFunction);
