...
typedef struct _HelloWorldClass
{
   ...
   */
  IDataEventListener_vtbl m_xDataEventListenerVTBL;
...
AManagedTask *HelloWorldTaskAlloc(const void *p_mx_drv_cfg)
{
  ...
  s_xTaskObj.m_xDpuListenerIF.vptr = &s_xTheClass.m_xDataEventListenerVTBL;
...
static sys_error_code_t HelloWorldTaskExecuteStepState1(AManagedTask *_this)
{
  ...
  SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("Hello IoT-Discovery!!\r\n"));
  __NOP();
  __NOP();
...
sys_error_code_t HelloWorldTask_vtblOnProcessedDataReady(IEventListener *_this, const DataEvent_t *p_evt)
{
  assert_param(_this != NULL);
  sys_error_code_t res = SYS_NO_ERROR_CODE;
